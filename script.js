...
document.addEventListener('DOMContentLoaded', function () {
    document.getElementById("balance").value = `Your will see your Eclipse balance here.`
    var web3 = new Web3(Web3.givenProvider);
    window.valid_address = false;

	document.getElementById("eclipse-wallet").addEventListener("input", async function() {
		...
		if (data.length >= 41) {
			let balance = await getEclipseBalance(data)
				.then(b => {
					...
					document.getElementById("balance").value = `Your Eclipse balance is: ${b}`
